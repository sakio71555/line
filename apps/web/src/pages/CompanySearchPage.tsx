    ...
    <main className="page-shell">
      ...
      <div className="company-list">
        {items.map((item, index) => (
          <CompanyCard key={`${item.company}-${item.name}-${item.mobile}-${index}`} item={item} />
        ))}
      </div>
    </main>
  );
...
function CompanyCard({ item }: { item: CompanySearchItem }) {
  return (
    <article className="company-card">
      <div className="company-card__header">
        <h2>{displayValue(item.company, "会社名未入力")}</h2>
        <p>{displayValue(item.region)}</p>
      </div>

      <dl className="company-card__grid">
        <CompanyField label="役職" value={item.title} />
        <CompanyField label="氏名" value={item.name} />
        ...
        <CompanyField label="支店情報" value={item.branches} wide />
