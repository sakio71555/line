...
リッチメニューの「使い方」、または個別チャットで以下のキーワードを送ると、Botが基本操作の案内テキストを返します。グループで送られた場合は、原則として送信者本人の個別チャットへ案内をPushします。
...
これらの投稿は `line_messages` に `message_type = 'help_request'` として保存され、案件や空車情報は作成しません。
...
`help_request` はBotが使い方案内を返すための投稿として扱い、`line_messages` にだけ保存します。グループでは送信者本人への個別Push、個別チャットではその場のreplyで案内を返します。
